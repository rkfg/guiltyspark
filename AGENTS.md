...
**Message buffering:** последовательные сообщения от одного участника в одной комнате объединяются. Первое сообщение индексируется сразу, второе и последующие от того же (room, user) — текст добавляется через `\n` и документ перезаписывается с тем же `EventID` первого сообщения. Каждый `(roomID, userID)` имеет независимый буфер.
...
- HTML escaping: в результатах поиска текст и описания изображений экранируются (`&`, `<`, `>`, `\n`) для безопасного рендеринга в HTML
- Эмодзи лупы (`🔍`, `\U0001f50d`) используется как префикс для описаний изображений в результатах поиска
